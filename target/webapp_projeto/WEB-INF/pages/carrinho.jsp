...
<main>
    <c:set var="precototal" value="${0}"></c:set>
    <div class="container">
        <h2>Conteudo do carrinho: </h2>
        <c:if test="${empty sessionScope.get('carrinho')}">
            <h4 style="color: grey">Nenhum livro adicionado no carrinho.</h4>
            <a href="carrinho?opcao=voltar" style="color: red; text-decoration: none">Voltar para a loja</a>
        </c:if>
        <c:if test="${not empty sessionScope.get('carrinho')}">
            <table class="table">
                <thead>
                <tr>
                ...
                <tbody>
                <c:forEach var = "livro" items="${sessionScope.get('carrinho')}">
                    <tr>
                        <td>${livro.nome}</td>
                        <td>${livro.autor}</td>
                        <td>R$<fmt:formatNumber value="${livro.preco}" type="number" maxFractionDigits="2" minFractionDigits="2"/></td>
                        <c:set var="precototal" value="${precototal + livro.preco}"></c:set>
                        <td>
                                <c:out value="${daoGenero.getGenero(livro.genero).nome}"/>
                        <td>
                            <a href="carrinho?opcao=excluir&&id=${livro.id}">RETIRAR LIVRO</a>
                        </td>
                    </tr>
                </c:forEach>
                </tbody>
            </table>
            <h3>Preco total: R$<fmt:formatNumber value="${precototal}" type="number" maxFractionDigits="2" minFractionDigits="2"/></h3>

            <div style="display: flex; flex-direction: column">
                <a href="carrinho?opcao=confirmar"><button type="button" class="btn btn-success">Confirmar Compra</button></a>
                <a href="carrinho?opcao=voltar" style="color: red; text-decoration: none">Voltar para a loja</a>
            </div>
        </c:if>
    </div>
